...
export const Results = () => {
  ...
  const location = useLocation();//returns the locetion i.e(/news. /images, /videos, etc)
          ...
          {results?.map(({ link, title, description }, index) => (
            <div key={index} className="md:w-2/5 w-full">
              <a href={link} target="_blank" rel="noreferrer">
                ...
                <h3 className="text-lg hover:underline dark:text-blue-300 text-blue-700  ">{description}</h3>
              </a>
            </div>
          ))}
        </div>
